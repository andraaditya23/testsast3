pipeline {
    ...
    post{
        success {
			discordSend link: env.BUILD_URL, result: currentBuild.currentResult, title: "${env.JOB_NAME} #${env.BUILD_NUMBER}", description: "Report Time ==> ${REPORT_TIME}", webhookURL: "${env.DISCORD_WEBHOOK_URL}"
			sh "exit 0"
		}
