pipeline {
    ...
    post{
        success {
			discordSend link: env.BUILD_URL, result: currentBuild.currentResult, title: "${env.JOB_NAME} #${env.BUILD_NUMBER}", description: "```elm\nCapital\nnormal\n123 123```", webhookURL: "${env.DISCORD_WEBHOOK_URL}"
			sh "exit 0"
		}
